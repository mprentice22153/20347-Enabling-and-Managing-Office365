...
12. To the right of **Active Directory synchronization**, verify that there is a **Manage** link (if activation was not yet completed this link would say "Set up"). If there is no **Manage**, click **Set up**, and verify that under **Directory synchronization** the **Directory sync status** is ** activated**.
